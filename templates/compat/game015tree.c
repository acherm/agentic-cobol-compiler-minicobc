...
static void print_decimal(char *buffer, size_t size, int num) {
    snprintf(buffer, size, "0.0%d", num);
}
...
static void print_node(int depth, int player, int num, int node_val,
    ...
    line[0] = '\0';
    print_decimal(decimal, sizeof(decimal), num);
    ...
    pos += snprintf(line + pos, sizeof(line) - (size_t)pos, "%s",
        is_last[depth] ? "+--- " : "|--- ");
    pos += snprintf(line + pos, sizeof(line) - (size_t)pos, "P%d:%s [%s]",
        player, decimal, value_label(node_val));
    if (is_terminal) {
...
static void print_avoid(int depth, const struct move_list *bad) {
    ...
    for (int i = 0; i < bad->count; ++i) {
        print_decimal(decimal, sizeof(decimal), bad->nums[i]);
        pos += snprintf(line + pos, sizeof(line) - (size_t)pos,
            (i == 0) ? " %s" : ", %s", decimal);
    }
...
int main(int argc, char **argv) {
    ...
    printf("Game of 0.15 - Optimal Play Tree [%s]\n", value_label(minimax(1)));
    print_tree(1);
