...
BUILD_DIR="$ROOT_DIR/build/chess-phase2"
...
"$ROOT_DIR/build/bin/minicobc" "$CHESS_DIR/src/perft.cob" "$BUILD_DIR/perft.c"
...
gcc -std=c11 -c "$BUILD_DIR/perft.c" -o "$BUILD_DIR/perft.o"
gcc -std=c11 "$ROOT_DIR/tests/chess/phase2_perft_harness.c" \
    "$BUILD_DIR/board.o" "$BUILD_DIR/fen.o" "$BUILD_DIR/attack.o" \
    "$BUILD_DIR/movegen.o" "$BUILD_DIR/makemove.o" "$BUILD_DIR/unmakemove.o" \
    "$BUILD_DIR/perft.o" \
    -o "$BUILD_DIR/minicobc-phase2"

"$BUILD_DIR/minicobc-phase2" > "$BUILD_DIR/minicobc.out"

cobc -x -free \
    -I "$CHESS_DIR" \
    "$ROOT_DIR/tests/chess/phase2_perft_ref.cob" \
    "$CHESS_DIR/src/board.cob" \
    "$CHESS_DIR/src/fen.cob" \
    "$CHESS_DIR/src/attack.cob" \
    "$CHESS_DIR/src/movegen.cob" \
    "$CHESS_DIR/src/makemove.cob" \
    "$CHESS_DIR/src/perft.cob" \
    -o "$BUILD_DIR/gnucobol-phase2"

"$BUILD_DIR/gnucobol-phase2" > "$BUILD_DIR/gnucobol.out"
...
echo "phase2 chess PERFT generic build matched GnuCOBOL"
